...
Antes de ejecutar los notebooks, cada pareja crea un \textbf{entorno virtual con la versión
correcta de Python} (3.10 o 3.11). Este es el paso más delicado: instalar las dependencias
sobre el Python equivocado ---el del sistema u otra versión--- provoca errores de
compatibilidad con TensorFlow.

\begin{enumerate}
  \item Abrir la carpeta del proyecto en \textbf{Visual Studio Code}.
  \item Crear el entorno: \emph{Command Palette} (\texttt{Ctrl+Shift+P}) $\rightarrow$
        \texttt{Python: Create Environment} $\rightarrow$ \texttt{Venv} $\rightarrow$ elegir
        un intérprete \textbf{Python 3.10} o \textbf{3.11}. Se crea la carpeta \texttt{.venv}.
  \item Abrir un notebook y, arriba a la derecha, \texttt{Select Kernel} y elegir ese
        entorno \texttt{.venv}.
  \item Instalar las dependencias \textbf{desde una celda del notebook} (así se instalan en
        el entorno seleccionado, no en otro Python):
\end{enumerate}
...
\noindent Verificar la versión antes de continuar; debe iniciar con 3.10 o 3.11:
