...
botellas provisto en \texttt{data/botellas/}; y los notebooks
\texttt{notebook1\_cnn\_desde\_cero.ipynb}, \texttt{notebook2\_data\_augmentation.ipynb}
y \texttt{notebook3\_clasificador\_tapitas.ipynb}. Cada notebook explica su código en
detalle; esta guía es la referencia teórica y la describe \emph{qué} se hace y
...
red colapsaría a una única transformación lineal.
...
preguntas conceptuales}. La rúbrica completa está en la \emph{Ficha de Evaluación}: la
Parte~1 aporta \textbf{10 puntos} y la Parte~2 otros 10, para una nota sobre 20.
...
y compilación de la arquitectura (bloques \texttt{Conv2D}\,+\,\texttt{MaxPooling2D},
\texttt{Flatten}, capa densa con \emph{dropout} y salida \emph{softmax}), entrenamiento,
evaluación con matriz de confusión y predicción sobre imágenes propias (Figuras~\ref{fig:curvas}
y~\ref{fig:confmat}).
